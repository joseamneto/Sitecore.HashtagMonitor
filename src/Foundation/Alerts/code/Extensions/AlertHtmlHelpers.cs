using GoHorse.Foundation.Alerts.Models;
using Sitecore;
using Sitecore.Data;
...
namespace GoHorse.Foundation.Alerts.Extensions
{
    public static class AlertHtmlHelpers
    {
        ...
        public static MvcHtmlString PageEditorError(this HtmlHelper helper, string errorMessage, string friendlyMessage, ID contextItemId, ID renderingId)
        {
            ...
            return Context.PageMode.IsNormal ? new MvcHtmlString(string.Empty) :
                helper.Partial(Constants.InfoMessageView, InfoMessage.Error(friendlyMessage));
        }
    }
}